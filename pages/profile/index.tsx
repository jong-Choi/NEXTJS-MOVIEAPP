...
const ProfilePage = () => {
  const {
    ...
    uid: myUid,
    nickname: myNickname,
  } = useTypedSelector((state) => state.authSlice.userProfile, shallowEqual);
  const myProfile = useTypedSelector(
  ...
  useEffect(() => {
    authService.onAuthStateChanged(async (user) => {
      ...
      const crrentUid = currentUser.uid;
      const profile = await fetchProfile(crrentUid);
      dispatch(setUserOjbect(currentUser));
      dispatch(setUserProfile(profile));
      if (!uid) setProfile(profile);
    });
  }, []);
